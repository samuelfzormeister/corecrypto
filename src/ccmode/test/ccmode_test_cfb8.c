...
void ccmode_cfb8_test_dump_state(cctest_ctx *tx)
{
    struct _ccmode_test_ctx *ctx = CCMODE_TEST_CTX(tx);
    uint8_t *state = (uint8_t *)CCMODE_TEST_CTX_KEY(cccfb8_ctx, ctx);
    ...
    for (cc_size i = 0; i < CCMODE_TEST_CTX_SCRATCH_SIZE(((struct ccmode_cfb8 *)ctx->mode)); i++) {
        if ((i % 8) == 0) {
            if (i > 0) {
                cc_printf("\n");
            }
...
int ccmode_cfb8_test_init(const struct cctest_info *info, cctest_ctx *ctx)
{
    struct _ccmode_test_ctx *tctx = (struct _ccmode_test_ctx *)ctx;
    const struct ccmode_cfb8 *mode = (const struct ccmode_cfb8 *)info->custom;
...
int ccmode_cfb8_encrypt_test_run(cctest_ctx *ctx)
{
    struct _ccmode_test_ctx *tctx = (struct _ccmode_test_ctx *)ctx;
    const struct ccmode_cfb8 *mode = (const struct ccmode_cfb8 *)tctx->mode;
    const struct ccmode_test_vector_info *vi = tctx->vi;
    void *scratch = CCMODE_TEST_CTX_SCRATCH_SPACE(tctx);
    cccfb8_ctx *cx = CCMODE_TEST_CTX_KEY(cccfb8_ctx, tctx);
    ...
    for (int i = 0; i < vi->nvectors; i++) {
        ...
        if (cc_cmp_safe(vec.text_length, vec.ciphertext, scratch) == 0) {
            ...
            ccmode_cfb8_test_dump_state(ctx);
...
int ccmode_cfb8_decrypt_test_run(cctest_ctx *ctx)
{
    struct _ccmode_test_ctx *tctx = (struct _ccmode_test_ctx *)ctx;
    const struct ccmode_cfb8 *mode = (const struct ccmode_cfb8 *)tctx->mode;
    const struct ccmode_test_vector_info *vi = tctx->vi;
...
void ccmode_cfb8_encrypt_test_factory(struct cctest_info *ti, const struct ccmode_cfb8 *mode, const char *name, struct ccmode_test_vector_info *vi)
{
    ...
    ti->init = ccmode_cfb8_test_init;
    ti->run = ccmode_cfb8_encrypt_test_run;
}
...
void ccmode_cfb8_decrypt_test_factory(struct cctest_info *ti, const struct ccmode_cfb8 *mode, const char *name, struct ccmode_test_vector_info *vi)
{
    ...
    ti->size = mode->size + CCMODE_TEST_CTX_SCRATCH_SIZE(mode);
    ti->init = ccmode_cfb8_test_init;
    ti->run = ccmode_cfb8_decrypt_test_run;
}
