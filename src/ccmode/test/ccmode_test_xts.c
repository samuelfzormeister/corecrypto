...
void ccmode_xts_test_dump_state(cctest_ctx *tx)
{
    struct _ccmode_test_ctx *ctx = CCMODE_TEST_CTX(tx);
    uint8_t *state = (uint8_t *)CCMODE_TEST_CTX_KEY(ccxts_ctx, ctx);
    ...
    for (cc_size i = 0; i < CCMODE_TEST_CTX_SCRATCH_SIZE(((struct ccmode_xts *)ctx->mode)); i++) {
        if ((i % 8) == 0) {
            if (i > 0) {
                cc_printf("\n");
            }
...
int ccmode_xts_test_init(const struct cctest_info *info, cctest_ctx *ctx)
{
    struct _ccmode_test_ctx *tctx = (struct _ccmode_test_ctx *)ctx;
    const struct ccmode_xts *mode = (const struct ccmode_xts *)info->custom;
...
int ccmode_xts_encrypt_test_run(cctest_ctx *ctx)
{
    struct _ccmode_test_ctx *tctx = (struct _ccmode_test_ctx *)ctx;
    const struct ccmode_xts *mode = (const struct ccmode_xts *)tctx->mode;
    const struct ccmode_test_vector_info *vi = tctx->vi;
    void *scratch = CCMODE_TEST_CTX_SCRATCH_SPACE(tctx);
    ccxts_ctx *cx = CCMODE_TEST_CTX_KEY(ccxts_ctx, tctx);
    ccxts_tweak *tweak = CCMODE_TEST_CTX_IV_SPACE(ccxts_tweak, tctx);

    for (int i = 0; i < vi->nvectors; i++) {
        struct ccmode_test_vector vec = vi->vectors[i];
        cc_size blocks = vec.text_length / ccxts_block_size(mode);
        const char *reason = "";
        void *twk = NULL;
        
        int ret = ccxts_init(mode, cx, vec.key_length, vec.key, vec.iv);
        cc_require_action(ret == 0, testfail, reason = "INIT FAIL");
        twk = ccxts_update(mode, cx, tweak, blocks, vec.plaintext, scratch);
        cc_require_action(twk != NULL, testfail, reason = "UPDATE FAIL");
        if (cc_cmp_safe(vec.text_length, vec.ciphertext, scratch) == 0) {
            cctest_trace_pass(CCTEST_SUBSYSTEM_MODE, tctx->ti->name, i+1);
            ccxts_ctx_clear(mode->size, cx);
            ...
            ccmode_xts_test_dump_state(ctx);
...
int ccmode_xts_decrypt_test_run(cctest_ctx *ctx)
{
    struct _ccmode_test_ctx *tctx = (struct _ccmode_test_ctx *)ctx;
    const struct ccmode_xts *mode = (const struct ccmode_xts *)tctx->mode;
    const struct ccmode_test_vector_info *vi = tctx->vi;
...
void ccmode_xts_encrypt_test_factory(struct cctest_info *ti, const struct ccmode_xts *mode, const char *name, struct ccmode_test_vector_info *vi)
{
    ti->name = name;
    ti->custom = mode;
    ti->custom1 = vi;
    ti->size = mode->size + CCMODE_TEST_CTX_SCRATCH_SIZE(mode);
    ti->init = ccmode_xts_test_init;
    ti->run = ccmode_xts_encrypt_test_run;
}

void ccmode_xts_decrypt_test_factory(struct cctest_info *ti, const struct ccmode_xts *mode, const char *name, struct ccmode_test_vector_info *vi)
{
    ti->name = name;
    ti->custom = mode;
    ti->custom1 = vi;
    ti->size = mode->size + CCMODE_TEST_CTX_SCRATCH_SIZE(mode);
    ti->init = ccmode_xts_test_init;
    ti->run = ccmode_xts_decrypt_test_run;
}
