...
void ccmode_ctr_test_dump_state(cctest_ctx *tx)
{
    struct _ccmode_test_ctx *ctx = CCMODE_TEST_CTX(tx);
    uint8_t *state = (uint8_t *)CCMODE_TEST_CTX_KEY(ccctr_ctx, ctx);
    ...
    for (cc_size i = 0; i < CCMODE_TEST_CTX_SCRATCH_SIZE(((struct ccmode_ctr *)ctx->mode)); i++) {
        if ((i % 8) == 0) {
            if (i > 0) {
                cc_printf("\n");
            }
...
int ccmode_ctr_test_init(const struct cctest_info *info, cctest_ctx *ctx)
{
    struct _ccmode_test_ctx *tctx = (struct _ccmode_test_ctx *)ctx;
    const struct ccmode_ctr *mode = (const struct ccmode_ctr *)info->custom;
...
int ccmode_ctr_encrypt_test_run(cctest_ctx *ctx)
{
    struct _ccmode_test_ctx *tctx = (struct _ccmode_test_ctx *)ctx;
    const struct ccmode_ctr *mode = (const struct ccmode_ctr *)tctx->mode;
    const struct ccmode_test_vector_info *vi = tctx->vi;
    void *scratch = CCMODE_TEST_CTX_SCRATCH_SPACE(tctx);
    ccctr_ctx *cx = CCMODE_TEST_CTX_KEY(ccctr_ctx, tctx);
    ...
    for (int i = 0; i < vi->nvectors; i++) {
        ...
        if (cc_cmp_safe(vec.text_length, vec.ciphertext, scratch) == 0) {
            ...
            ccmode_ctr_test_dump_state(ctx);
...
int ccmode_ctr_decrypt_test_run(cctest_ctx *ctx)
{
    struct _ccmode_test_ctx *tctx = (struct _ccmode_test_ctx *)ctx;
    const struct ccmode_ctr *mode = (const struct ccmode_ctr *)tctx->mode;
    const struct ccmode_test_vector_info *vi = tctx->vi;
...
void ccmode_ctr_encrypt_test_factory(struct cctest_info *ti, const struct ccmode_ctr *mode, const char *name, struct ccmode_test_vector_info *vi)
{
    ti->name = name;
    ti->custom = mode;
    ti->custom1 = vi;
    ti->size = mode->size + CCMODE_TEST_CTX_SCRATCH_SIZE(mode);
    ti->init = ccmode_ctr_test_init;
    ti->run = ccmode_ctr_encrypt_test_run;
}

void ccmode_ctr_decrypt_test_factory(struct cctest_info *ti, const struct ccmode_ctr *mode, const char *name, struct ccmode_test_vector_info *vi)
{
    ti->name = name;
    ti->custom = mode;
    ti->custom1 = vi;
    ti->size = mode->size + CCMODE_TEST_CTX_SCRATCH_SIZE(mode);
    ti->init = ccmode_ctr_test_init;
    ti->run = ccmode_ctr_decrypt_test_run;
}
