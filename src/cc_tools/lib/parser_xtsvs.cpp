...
//
void parser::parse_xtsvs(std::stringstream &stream)
{
    osl::log(osl::debug, "parsing xtsvs...");
    std::string line;
    std::string key;
    std::string iv;
    ...
    auto encrypt = std::make_shared<xtsvs_test>(_basename + "_encrypt", aesvs_test::xts);
    auto decrypt = std::make_shared<xtsvs_test>(_basename + "_decrypt", aesvs_test::xts);
    ...
    while (std::getline(stream, line)) {
        ...
        if (line.find("i") != std::string::npos) {
            ...
            osl::log(osl::debug, "parser: found tweak val");
        }
        ...
        if (curEnc) {
            ...
            if (line.find("CT") != std::string::npos) {
                ...
                auto vector = std::make_shared<xtsvs_vector>(key, iv, plaintext, ciphertext);
