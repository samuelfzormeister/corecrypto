...
#else
    static struct ccmode_xts xts_decrypt_mode;
    const struct ccmode_ecb *ecb = ccaes_ecb_decrypt_mode();
    const struct ccmode_ecb *ecb_encrypt = ccaes_ecb_encrypt_mode();

    ccmode_factory_xts_decrypt(&xts_decrypt_mode, ecb, ecb_encrypt);
    return &xts_decrypt_mode;
#endif
}
