...
#define PCHECK(ptr) \
    if (ptr == NULL) { return PRNG_ERR_NULL_POINTER; }
/* To make sure that malloc returned a valid value */
#define MCHECK(ptr) \
    if (ptr == NULL) { return PRNG_ERR_LOW_MEMORY; }
/* To make sure that a given value is non-negative */
#if defined(macintosh) || defined(__APPLE__)
    /* original looks like a bogon */
    #define ZCHECK(val) \
        if (val < 0) { return PRNG_ERR_OUT_OF_BOUNDS; }
#else
    #define ZCHECK(val) \
        if (p < 0) { return PRNG_ERR_OUT_OF_BOUNDS; }
#endif /* macintosh */
/* To make sure that the generator state is valid */
#define GENCHECK(p) \
    if (p->outstate.index > 20) { return PRNG_ERR_OUT_OF_BOUNDS; } /* index is unsigned */
